...
elseif uex=="Trigo"
    ...
    dbc = [2 1 "cos(pi*y)"   ;2 2 "sin(pi*y)-1" ;
           3 1 "sin(pi*x)-1" ;3 2 "cos(pi*x)"   ;
           4 1 "cos(pi*y)"   ;4 2 "sin(pi*y)-1" ;
           5 1 "sin(pi*x)-1" ;5 2 "cos(pi*x)"   ];
    
elseif uex == "Expo"
    
    dbc = [2 1 "exp(-y)" ; 2 2 "exp(y)"  ;
           3 1 "exp(-x)" ; 3 2 "exp(x)"  ;
           4 1 "exp(y)"  ; 4 2 "exp(-y)" ;
           5 1 "exp(x)"  ; 5 2 "exp(-x)" ];
...
elseif uex=="Poly4_mixte_3"
    ...
    tbc = [2 1 "2*(lambda+2*mu)*(y*y-1)" ; 2 2 "2*mu*(y*y-1)"             ;
           4 1 "2*(lambda+2*mu)*(y*y-1)" ; 4 2 "2*mu*(y*y-1)"             ;
           5 1 "2*mu*(x*x-1)"            ; 5 2 "2*(lambda+2*mu)*(x*x-1)" ];
